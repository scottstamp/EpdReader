...
static_assert(PINS_COUNT == 38,
              "Custom variant not loaded! PINS_COUNT must be 38.");
...
bool isSystemInReaderMode() { return currentState == STATE_READER; }
...
void bleProgressCallback(const String &status, int bytesReceived,
                         bool finished);
void handleUsbMsc();
...
extern bool useSlowFullRefresh;
int lockUnlockPressCount = 0;
...
String getCoverArtPath(const String& filename) {
  if (!filename.startsWith("[SD]")) {
    return "";
  }
  String path = filename.substring(4); // Remove [SD]
  int lastSlash = path.lastIndexOf('/');
  if (lastSlash >= 0) {
    // Chapterized book, folder is path.substring(0, lastSlash + 1)
    String folder = path.substring(0, lastSlash + 1);
    return "/" + folder + "cover.mono";
  } else {
    // Flat book file, e.g. "The_Compound.txt"
    String baseName = path;
    if (baseName.endsWith(".txt")) {
      baseName = baseName.substring(0, baseName.length() - 4);
    }
    // Check if /BaseName/cover.mono exists
    String opt1 = "/" + baseName + "/cover.mono";
    if (StorageManager::getInstance().sdExists(opt1)) {
      return opt1;
    }
    // Otherwise try /BaseName.mono
    return "/" + baseName + ".mono";
  }
}

void parseTitleAndAuthor(const String& fullTitle, String& title, String& author) {
  int hyphenIdx = fullTitle.indexOf(" - ");
  if (hyphenIdx >= 0) {
    title = fullTitle.substring(0, hyphenIdx);
    author = fullTitle.substring(hyphenIdx + 3);
  } else {
    title = fullTitle;
...
void drawLockscreen() {
  String title = "";
  String author = "";
  parseTitleAndAuthor(activeBookTitle, title, author);

  String chapter = "—";
  if (isActiveBookChapterized) {
    chapter = bookChapters[activeChapterIdx].title;
  }

  // Calculate current page/percentage
  int percent = 0;
  uint32_t fileOffset = currentPageOffset;
  if (isActiveBookChapterized) {
    fileOffset = currentPageOffset & 0x00FFFFFF;
  }
  uint32_t totalSize = activeBookSize > 0 ? activeBookSize : 1;
  percent = (fileOffset * 100) / totalSize;
  String pageStr = String(percent) + "%";

  // Get cover art path
  String coverPath = getCoverArtPath(activeBookFilename);

  LockscreenView lockView(title, author, chapter, pageStr, coverPath, lockUnlockPressCount);
  DisplayManager::getInstance().draw(lockView);
}
...
bool loadBookChapters(const String &bookPath) {
  ...
  FsFile indexFile =
      StorageManager::getInstance().openSDBook(indexPath, O_RDONLY);
  if (!indexFile) {
  ...
  while (indexFile.available() && bookChapterCount < MAX_CHAPTERS) {
    ...
    if (filename.length() > 0) {
      ...
      bookChapters[bookChapterCount].size =
          0; // Defer size loading until active
...
void setup() {
  ...
  // 2. Read and clear GPREGRET immediately to detect sleep wake-up before other
  // libraries run
  ...
  Serial.print("[T+");
  Serial.print(t - t0);
  Serial.println("ms] Initializing battery...");
  ...
  Serial.print("[T+");
  Serial.print(t - t0);
  Serial.println("ms] Initializing Storage (InternalFS)...");
  ...
  Serial.print("[T+");
  Serial.print(t - t0);
  Serial.println("ms] Migrating InternalFS -> SD...");
  StorageManager::getInstance().migrateInternalFSFiles();

  t = millis();
  Serial.print("[T+");
  Serial.print(t - t0);
  Serial.println("ms] Reading stats from SD...");
  ...
  Serial.print("[T+");
  Serial.print(t - t0);
  Serial.println("ms] Initializing Buttons...");
  ButtonManager::getInstance().begin();

  t = millis();
  Serial.print("[T+");
  Serial.print(t - t0);
  Serial.println("ms] Initializing Display (EPD)...");
  ...
  // t = millis();
  // Serial.print("[T+"); Serial.print(t - t0); Serial.println("ms] Initializing
  // BLE..."); BleManager::getInstance().begin(bleStateCallback,
  // bleProgressCallback); BleManager::getInstance().startAdvertising();

  t = millis();
  Serial.print("[T+");
  Serial.print(t - t0);
  Serial.println("ms] Setup subsystems done.");
  lastActivityTime = millis();
  Serial.print("[T+");
  Serial.print(millis() - t0);
  Serial.println("ms] Setup completed successfully.");
  // Check if we have a saved sleep state (now on SD)
  uint32_t t_before_sleep = millis();
  bool restoredFromSleepState = false;
  bool restoreToLockscreen = false;
  int savedState = 0, savedMenuIdx = 0, savedBookIdx = 0, savedSdBookIdx = 0;
  int savedPcBookIdx = 0, savedChapterIdx = 0, savedReaderMenuIdx = 0,
      savedActiveChapterIdx = 0;
  String savedActiveBook = "";
  if (StorageManager::getInstance().readSleepState(
          savedState, savedMenuIdx, savedBookIdx, savedSdBookIdx,
          savedPcBookIdx, savedChapterIdx, savedReaderMenuIdx, savedActiveBook,
          savedActiveChapterIdx)) {
    // Clear it so it's a one-time restore
    StorageManager::getInstance().clearSleepState();

    if (savedState == STATE_LOCKSCREEN) {
      restoreToLockscreen = true;
      savedState = STATE_READER;
    }
  ...
  Serial.print("[T+");
  Serial.print(t_after_sleep - t0);
  Serial.println("ms] Sleep state restore done.");
  ...
  if (!restoredFromSleepState) {
    ...
    int histCount = 0;
    if (StorageManager::getInstance().readProgress(savedBook, savedOffset, hist,
                                                   HISTORY_SIZE, histCount) &&
        savedBook.length() > 0) {
      ...
      if (savedBook.startsWith("[SD]")) {
        ...
        if (slashIdx >= 0) {
          String folderPath =
              savedBook.substring(0, slashIdx + 1); // e.g. "[SD]The_Hobbit/"
          if (loadBookChapters(folderPath)) {
            ...
            if (foundIdx >= 0) {
              ...
              if (bookChapters[activeChapterIdx].size == 0) {
                FsFile chapFile = StorageManager::getInstance().openSDBook(
                    bookChapters[activeChapterIdx].filename, O_RDONLY);
                if (chapFile) {
              ...
              // Normalize: ensure currentPageOffset always has chapter index in
              // upper byte. Old saves may have had plain file offsets without
              // chapter encoding.
              uint32_t fileOffsetOnly = savedOffset & 0x00FFFFFF;
              currentPageOffset =
                  ((uint32_t)activeChapterIdx << 24) | fileOffsetOnly;

              DisplayManager::getInstance().clearHistory();
              if (histCount > 0) {
                // Re-encode any history entries that lack a chapter index
                // (entries from old saves will have 0 in the upper byte if on
                // chapter 0, or garbage if they were plain offsets — normalize
                // them all)
                uint32_t normHist[HISTORY_SIZE];
                for (int i = 0; i < histCount; i++) {
                  // If the saved history entry has 0 in the chapter byte but
                  // we're on a chapter > 0, it was a plain offset — re-encode
                  // it. If it already has a chapter index encoded, leave it
                  // as-is.
                  uint32_t hEntry = hist[i];
                  if ((hEntry >> 24) == 0 && activeChapterIdx > 0) {
                    hEntry = ((uint32_t)activeChapterIdx << 24) |
                             (hEntry & 0x00FFFFFF);
                  }
                  normHist[i] = hEntry;
                }
                DisplayManager::getInstance().setHistoryOffsets(normHist,
                                                                histCount);
              }
              // Always ensure the current page is the top of the history stack
              // so PREV navigates correctly. pushHistory deduplicates, so this
              // is a no-op if currentPageOffset is already the last entry.
              DisplayManager::getInstance().pushHistory(currentPageOffset);

              transitionTo(restoreToLockscreen ? STATE_LOCKSCREEN : STATE_READER);
              return;
            }
          }
        }
        ...
        FsFile file =
            StorageManager::getInstance().openSDBook(realFilename, O_RDONLY);
        if (file) {
          activeBookFilename = savedBook;
          ...
          // Ensure current page is top of history stack for correct PREV nav.
          DisplayManager::getInstance().pushHistory(currentPageOffset);
        ...
        if (file) {
          ...
          // Push very first page offset to history to initialize back
          // navigation
          DisplayManager::getInstance().clearHistory();
          if (histCount > 0) {
            DisplayManager::getInstance().setHistoryOffsets(hist, histCount);
...
void loop() {
  ...
  // Do not sleep while connected to BLE, active upload stream, or if Serial is
  // active (USB connected and monitor open)
  bool isBleConnected = BleManager::getInstance().isConnected() ||
                        BleManager::getInstance().isCentralConnected();
  ...
  if (currentState != STATE_USB_MSC && !isBleBusy && !usbBlocksSleep &&
...
void transitionTo(SystemState newState) {
  ...
  // Manage reading session: close any active session before leaving
  // STATE_READER
  ...
  // Enable PC stream bypass in BleManager if we are in PC Browse or Reader with
  // a BLE book
  bool isBleBook = activeBookFilename.startsWith("[BLE]");
  bool pcStream =
      (newState == STATE_PC_BROWSE) || (newState == STATE_READER && isBleBook);
  ...
  if (newState == STATE_MENU) {
    ...
    if (bookCount == 0) {
      ...
      for (int i = 0; i < bookCount; i++) {
        ...
        if (cleanNames[i].endsWith(".txt")) {
          cleanNames[i] =
              cleanNames[i].substring(0, cleanNames[i].length() - 4);
        }
    ...
    bool isConnected = BleManager::getInstance().isCentralConnected() ||
    ...
    if (isConnected) {
      if (resumeOnConnect) {
        ...
        activeBookSize =
            BleManager::getInstance().requestBookSize(realFilename);
        activeBookTitle = realFilename;
        if (activeBookTitle.endsWith(".txt")) {
          activeBookTitle =
              activeBookTitle.substring(0, activeBookTitle.length() - 4);
        }
        ...
        uint32_t hist[HISTORY_SIZE];
        int histCount = 0;
        if (StorageManager::getInstance().readProgress(
                savedBook, savedOffset, hist, HISTORY_SIZE, histCount) &&
            histCount > 0) {
          DisplayManager::getInstance().setHistoryOffsets(hist, histCount);
        } else {
          char tempBuf[128];
          int len = BleManager::getInstance().requestBookText(
              realFilename, 0, tempBuf, sizeof(tempBuf) - 1);
          tempBuf[len] = '\0';
      ...
      if (BleManager::getInstance().requestBookList(pcBookList, MAX_BOOKS,
                                                    pcBookCount)) {
        pcListFetched = true;
        if (pcBookCount == 0) {
          ...
          MenuView menu("Select PC Book", cleanNames, pcBookCount,
                        selectedPcBookIdx);
          DisplayManager::getInstance().draw(menu);
        }
      } else {
        MessageView errMsg(
            "Error", "Failed to get book list.\nReturning to menu...", true);
    ...
    if (oldState == STATE_LOCKSCREEN) {
      DisplayManager::getInstance().requestFullRefresh();
      useSlowFullRefresh = true;
    }
    // Render current page
    if (isActiveBookChapterized) {
      uint32_t fileOffset = currentPageOffset & 0x00FFFFFF;
      ReaderView reader(activeBookFilename, fileOffset, nextPageOffset, true,
                        bookChapters[activeChapterIdx].title,
                        bookChapters[activeChapterIdx].size);
    ...
    MessageView msg(
        "BLE Upload",
        "Device Name: nRF_Epd_Reader\n\nStatus: Advertising BLE...\nUse nRF "
        "Connect or NUS App\nto stream a book text file.",
        false);
    DisplayManager::getInstance().draw(msg);
    ...
    sdBookCount =
        StorageManager::getInstance().listSDBooks(sdBookList, MAX_BOOKS);
    ...
    if (sdBookCount == 0) {
      ...
      for (int i = 0; i < sdBookCount; i++) {
        ...
        if (cleanNames[i].endsWith(".txt")) {
          ...
          cleanNames[i] =
              cleanNames[i].substring(0, cleanNames[i].length() - 1);
        }
        cleanNames[i].replace("_", " ");
      }
      cleanNames[sdBookCount] = "[Back]";
      MenuView menu("Select Book", cleanNames, sdBookCount + 1,
                    selectedSdBookIdx);
      DisplayManager::getInstance().draw(menu);
    }
    ...
    if (StorageManager::getInstance().enableUSBMSC(true)) {
      MessageView msg("USB SD Reader",
                      "SD card exposed over USB.\n\nPress select to exit.",
                      false);
      DisplayManager::getInstance().draw(msg);
    } else {
      MessageView msg("USB Error",
                      "Failed to expose SD card.\nReturning to menu...", true);
      DisplayManager::getInstance().draw(msg);
      delay(2000);
      transitionTo(STATE_MENU);
    }
    ...
    MenuView menu("Select Chapter", chapterMenuOptions, bookChapterCount,
                  selectedChapterIdx);
    DisplayManager::getInstance().draw(menu);
    ...
    if (selection == "Resume Reading") {
      ...
      if (StorageManager::getInstance().readProgress(
        ...
        if (savedBook.startsWith("[BLE]")) {
          ...
          String realFilename = savedBook.substring(4);
          // Check if this is a chapterized book (path contains a slash, e.g.
          // "The_Compound/p1c6.txt")
          int slashIdx = realFilename.indexOf('/');
          if (slashIdx > 0) {
            // Chapterized book – restore full chapter state
            String folderPath =
                "[SD]" + realFilename.substring(0, slashIdx + 1);
            if (loadBookChapters(folderPath)) {
              int foundIdx = 0;
              for (int i = 0; i < bookChapterCount; i++) {
                if (bookChapters[i].filename == realFilename) {
            ...
            FsFile file = StorageManager::getInstance().openSDBook(realFilename,
                                                                   O_RDONLY);
            if (file) {
              activeBookFilename = savedBook;
              activeBookSize = file.size();
              ...
              DisplayManager::getInstance().clearHistory();
              if (histCount > 0) {
                DisplayManager::getInstance().setHistoryOffsets(hist,
                                                                histCount);
              } else {
                uint32_t textStart = activeBookTitle.length() + 1;
                DisplayManager::getInstance().pushHistory(textStart);
      ...
      MessageView formattingMsg(
          "Formatting", "Clearing storage files\nPlease wait...", false);
      DisplayManager::getInstance().draw(formattingMsg);
      StorageManager::getInstance().clearStorage();
      DisplayManager::getInstance()
          .saveSettings(); // Restore font preferences to new filesystem
      MessageView clearedMsg(
          "Storage Cleared",
          "All books and reading\nprogress have been deleted.", false);
...
void handleBookList() {
  ...
  if (bookCount == 0) {
    if (select == BTN_CLICK || select == BTN_LONG_PRESS || prev == BTN_CLICK ||
        next == BTN_CLICK) {
      transitionTo(STATE_MENU);
    }
    return;
  ...
  if (prev == BTN_CLICK) {
    ...
    LfsFile file =
        StorageManager::getInstance().openBook(activeBookFilename, "r");
    if (file) {
      activeBookSize = file.size();
      activeBookTitle = file.readStringUntil('\n');
      activeBookTitle.trim();
      ...
      // Start at beginning (which will dynamically resolve past the metadata
      // first line)
      currentPageOffset = 0;
      ...
      // Write progress
      StorageManager::getInstance().writeProgress(activeBookFilename,
                                                  textStart);
...
void handleReader() {
  ...
  if (activeBookFilename.startsWith("[BLE]") && !isConnected) {
    MessageView msg("Connection Lost",
  ...
  if (next == BTN_CLICK || bleRequestNextPage) {
    ...
    if (isActiveBookChapterized) {
      ...
      if (nextOffsetOnly < activeBookSize &&
          nextPageOffset != (currentPageOffset & 0x00FFFFFF)) {
        // Re-encode chapter index into the stored offset so history always
        // carries chapter info
        currentPageOffset =
            ((uint32_t)activeChapterIdx << 24) | (nextOffsetOnly & 0x00FFFFFF);
        DisplayManager::getInstance().pushHistory(currentPageOffset);
        StorageManager::getInstance().writeProgress(activeBookFilename,
                                                    currentPageOffset);
        transitionTo(STATE_READER);
        ...
        if (activeChapterIdx + 1 < bookChapterCount) {
          ...
          MessageView msg("The End",
                          "You have finished reading:\n" + activeBookTitle +
                              "\n\nPress select to return.",
                          false);
          DisplayManager::getInstance().draw(msg);
          currentPageOffset = ((uint32_t)activeChapterIdx << 24) |
                              (nextPageOffset & 0x00FFFFFF);
        }
      }
    } else {
      if (nextPageOffset < activeBookSize &&
          nextPageOffset != currentPageOffset) {
...
void bleProgressCallback(const String &status, int bytesReceived,
  ...
  if (finished) {
    ...
    if (currentState == STATE_BLE_UPLOAD) {
      ...
      MessageView msg("Upload Complete",
                      status + "\nSize: " + sizeMsg +
...
void enterDeepSleep() {
  ...
  // 4. Re-configure the button GPIOs to hardware-sense LOW levels to trigger
  // wakeup.
...
void drawReaderMenu() {
  ...
  bool hasBmk =
      StorageManager::getInstance().readBookmark(activeBookFilename, bmkOffset);
  if (hasBmk) {
    int bmkPercent = 0;
    ...
    if (isActiveBookChapterized && bmkChapterIdx < (uint32_t)bookChapterCount) {
      if (bookChapters[bmkChapSize == activeBookSize ? bmkChapterIdx
                                                     : bmkChapterIdx]
              .size == 0) {
        FsFile chapFile = StorageManager::getInstance().openSDBook(
            bookChapters[bmkChapterIdx].filename, O_RDONLY);
        if (chapFile) {
    ...
    if (isActiveBookChapterized) {
      readerMenuOptions[2] = "Go to Bookmark (Ch " + String(bmkChapterIdx + 1) +
                             ", " + String(bmkPercent) + "%)";
    } else {
  ...
  readerMenuOptions[idx++] =
      "Orientation: " +
      String(DisplayManager::getInstance().isFlipped() ? "Flipped" : "Normal");
  readerMenuOptions[idx++] = "Refresh Display";
  ...
  if (isActiveBookChapterized) {
    header =
        bookChapters[activeChapterIdx].title + " (" + String(percent) + "%)";
  } else {
    header = "Reader Menu (" + String(percent) + "%)";
  }

  String sessionStr = "Session: " + formatDuration(currentSessionSecondsLive());
  MenuView menu(header, readerMenuOptions, readerMenuCount,
                selectedReaderMenuIdx, sessionStr);
  DisplayManager::getInstance().draw(menu);
}
...
void handleReaderMenu() {
  ...
  if (prev == BTN_CLICK) {
    lastActivityTime = millis();
    selectedReaderMenuIdx =
        (selectedReaderMenuIdx - 1 + readerMenuCount) % readerMenuCount;
    drawReaderMenu();
  } else if (next == BTN_CLICK) {
    lastActivityTime = millis();
    ...
    if (selectedReaderMenuIdx == idx++) {
      ...
      StorageManager::getInstance().writeBookmark(activeBookFilename,
                                                  currentPageOffset);
      MessageView msg("Bookmark Set",
      ...
      uint32_t hist[HISTORY_SIZE];
      int histCount = 0;
      if (StorageManager::getInstance().readBookmark(
              activeBookFilename, bmkOffset, hist, HISTORY_SIZE, histCount)) {
        ...
        MessageView msg("No Bookmark",
                        "No bookmark has been\nset for this book.", true);
        DisplayManager::getInstance().draw(msg);
        delay(1200);
        drawReaderMenu();
      }
      ...
      // Force a full refresh of the current page to clear ghosting
      DisplayManager::getInstance().requestFullRefresh();
      transitionTo(STATE_READER);
    } else if (selectedReaderMenuIdx == idx++) {
...
void handleLockscreen() {
  ...
  if (prev == BTN_CLICK || next == BTN_CLICK) {
    lastActivityTime = millis();
  } else if (select == BTN_CLICK) {
    lastActivityTime = millis();
    lockUnlockPressCount++;
    if (lockUnlockPressCount >= 3) {
      lockUnlockPressCount = 0;
      transitionTo(STATE_READER);
    }
  }
...
void handleSerialUpload() {
  if (Serial.available()) {
    ...
    if (cmd.startsWith("USB_UPLOAD:")) {
      ...
      // Stop BLE advertising during USB upload to prevent SoftDevice radio
      // scheduling and interrupt conflicts
      BleManager::getInstance().stopAdvertising();
          ...
          if (count > 0) {
            ...
            for (int i = 0; i < count; i++) {
              ...
              if (sectorCount >= sizeof(sectorBuf)) {
                bool writeOk = StorageManager::getInstance().writeBookChunk(
                    sectorBuf, sectorCount);
                if (!writeOk) {
                  Serial.print("[Device Debug] USB Serial upload: "
                               "writeBookChunk failed at ");
      ...
      if (sectorCount > 0) {
        ...
        if (!writeOk) {
          Serial.println(
              "[Device Debug] USB Serial upload: Final buffer flush failed.");
        }
      }
...
void handlePcBrowse() {
  ...
  if (!isConnected) {
    ...
    bool nowConnected = BleManager::getInstance().isCentralConnected() ||
                        BleManager::getInstance().isConnected();
    if (nowConnected) {
    ...
    if (millis() - scanStartTime > 15000) { // 15-second scan timeout
      BleManager::getInstance().stopScanning();
      scanStartTime = 0;
      MessageView msg("Not Found",
                      "EpdBookServer not found.\nReturning to menu...", true);
      DisplayManager::getInstance().draw(msg);
      delay(2000);
      transitionTo(STATE_MENU);
    }
  ...
  if (prev == BTN_CLICK) {
    ...
    char tempBuf[128];
    int len = BleManager::getInstance().requestBookText(
        selectedBook, 0, tempBuf, sizeof(tempBuf) - 1);
    tempBuf[len] = '\0';
    char *newlinePtr = strchr(tempBuf, '\n');
    uint32_t textStart = 0;
    if (newlinePtr) {
...
void handleSdBrowse() {
  ...
  if (prev == BTN_CLICK) {
    lastActivityTime = millis();
    selectedSdBookIdx =
        (selectedSdBookIdx - 1 + sdBookCount + 1) % (sdBookCount + 1);
    ...
    if (StorageManager::getInstance().readProgress(savedBook, savedOffset, hist,
                                                   HISTORY_SIZE, histCount)) {
      if (savedBook == fullBookPath ||
          (fullBookPath.endsWith("/") && savedBook.startsWith(fullBookPath))) {
        hasProgress = true;
      }
    }
    ...
    bool hasBmk = false;
    if (!hasProgress) {
      hasBmk = StorageManager::getInstance().readBookmark(
          fullBookPath, bmkOffset, hist, HISTORY_SIZE, histCount);
    }
    ...
    if (filename.endsWith("/")) {
        ...
        if (hasProgress) {
          ...
          if (histCount > 0) {
            ...
            for (int i = 0; i < histCount; i++) {
              ...
              if ((hEntry >> 24) == 0 && activeChapterIdx > 0) {
                hEntry =
                    ((uint32_t)activeChapterIdx << 24) | (hEntry & 0x00FFFFFF);
              }
              normHist[i] = hEntry;
            }
            DisplayManager::getInstance().setHistoryOffsets(normHist,
          ...
          if (bookChapters[0].size == 0) {
            FsFile chapFile = StorageManager::getInstance().openSDBook(
                bookChapters[0].filename, O_RDONLY);
            if (chapFile) {
      ...
      FsFile file =
          StorageManager::getInstance().openSDBook(filename, O_RDONLY);
      if (file) {
...
void handleChapterList() {
  ...
  if (prev == BTN_CLICK) {
    lastActivityTime = millis();
    selectedChapterIdx =
        (selectedChapterIdx - 1 + bookChapterCount) % bookChapterCount;
...
void drawFontSettingsMenu() {
  ...
  if (lineSpacing == SPACING_NORMAL)
    spacingName = "Normal";
  else if (lineSpacing == SPACING_RELAXED)
    spacingName = "Relaxed";
  ...
  MenuView menu("Font Settings", fontMenuOptions, fontMenuCount,
                selectedFontMenuIdx);
  DisplayManager::getInstance().draw(menu);
}
...
void handleFontSettings() {
  ...
  if (prev == BTN_CLICK) {
    lastActivityTime = millis();
    selectedFontMenuIdx =
        (selectedFontMenuIdx - 1 + fontMenuCount) % fontMenuCount;
    drawFontSettingsMenu();
  } else if (next == BTN_CLICK) {
    lastActivityTime = millis();
